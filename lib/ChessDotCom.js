...
/*
 Free-text API fields (display names, usernames) are rendered by QML Text
 items. Strip markup, entities and control characters and cap length so a
 hostile payload can never smuggle rich text or huge strings into UI.
*/
...
function parseProfile(raw) {
  ...
  return {
    name: cleanText(raw.name || raw.username || "", 80),
  ...
  for (var i = rawArray.length - 1; i >= 0 && games.length < max; i--) {
    try {
      ...
      games.push({
        ...
        opponent: cleanText(opp.username, 64),
