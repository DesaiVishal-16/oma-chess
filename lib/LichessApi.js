...
function parseProfile(raw) {
  ...
  return {
    id: String(raw.id || ""),
    name: String(raw.username || raw.id || ""),
...
function parseGamesNdjson(text, usernameLower, max) {
  ...
  for (var i = 0; i < lines.length && games.length < max; i++) {
    ...
    try {
      ...
      games.push({
        result: result,
        opponent: opp.user && opp.user.name ? String(opp.user.name) : "Anonymous",
...
function parseUpcomingTournaments(raw, now, horizonMs) {
  ...
  for (var gi = 0; gi < groups.length; gi++) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      var clock = t.clock || {}
      var perfName = t.perf && t.perf.name ? String(t.perf.name) : ""
      out.push({
        id: String(t.id),
        name: String(t.fullName || t.name || t.id),
        perf: perfName,
