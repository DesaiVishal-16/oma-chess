...
function perfStatsUrl(user, perf) {
  return "https://lichess.org/api/user/" + encodeURIComponent(user) +
    "/perf/" + encodeURIComponent(perf)
}
...
/*
   /api/user/{username}/perf/{perf} → lifetime W/L/D for one format.
   The `stat.count` block holds aggregate totals: { all, rated, win, loss, draw }.
   Returns null when the block is missing (e.g. no rated games in that format).
 */
function parsePerfStats(raw) {
  if (!raw || typeof raw !== "object") return null
  var stat = raw.stat || {}
  var count = stat.count || {}
  var win = Number(count.win) || 0
  var loss = Number(count.loss) || 0
  var draw = Number(count.draw) || 0
  if (win + loss + draw === 0) return null
  return { wins: win, losses: loss, draws: draw, all: Number(count.all) || win + loss + draw }
}
